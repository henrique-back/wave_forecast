...
def parse_args():
    ...
    p.add_argument('--target', default='density', choices=['density', 'hs', 'shape', 'combined'],
...
def save_metrics_json(project_root, experiment, target, lead, seed,
                      channel_set, aux_set, metrics):
...
def plot_density_sample(idx, freqs_np, steps, pred_phys, true_phys, pers_phys,
                        hs_pred, hs_true, hs_pers, title_prefix='Sample'):
...
def print_bulk_table(idx, lead_time_steps, hs_true, hs_pred, hs_pers, tm02_true=None, tm02_pred=None):
    print(f"\nSample {idx}:")
...
def run_single(args, project_root, density, alpha_1, alpha_2, r_1, wind, freqs, freqs_np, device):
    ckpt, results_folder = find_checkpoint(project_root, args.experiment, args.target,
                                           1, args.lead, args.seed)
    ...
    _, _, test_loader, _, _, _, _ = _prepare_dataloaders(
        density, alpha_1, alpha_2, r_1,
        params['seq_len'], lead_time_steps, params['batch_size'],
        args.target, shuffle_seed=args.seed, wind=wind,
    ...
    if args.save_metrics:
        ...
        full_metrics = evaluate(model, test_loader, device, freqs,
                                lead_time=lead_time_steps, freq_means=freq_means)
        save_metrics_json(project_root, args.experiment, args.target,
                          args.lead, args.seed, args.channel_set, args.aux_set, full_metrics)
    ...
    with torch.no_grad():
        for idx in indices:
            ...
            y_pred = model.infer(X, freqs, lead_time_steps, freq_means=freq_means, aux=aux)
            ...
            if args.target == 'density':
                ...
                print_bulk_table(idx, lead_time_steps, hs_true, hs_pred, hs_pers, tm02_true, tm02_pred)
                ...
                n_snap = min(args.n_steps, lead_time_steps)
                steps = sorted(set(np.linspace(0, lead_time_steps - 1, n_snap, dtype=int).tolist()))
                fig = plot_shape_sample(idx, freqs_np, steps, pred_shape, true_shape, pers_shape)
...
def run_combined(args, project_root, density, alpha_1, alpha_2, r_1, wind, freqs, freqs_np, device):
    hs_ckpt, hs_folder = find_checkpoint(project_root, args.experiment, 'hs',
                                        1, args.lead, args.seed)
    shape_ckpt, shape_folder = find_checkpoint(project_root, args.experiment, 'shape',
                                               1, args.lead, args.seed)
    ...
    _, _, hs_test_loader, _, _, _, _ = _prepare_dataloaders(
        density, alpha_1, alpha_2, r_1, hs_params['seq_len'], lead_time_steps,
        hs_params['batch_size'], 'hs', shuffle_seed=args.seed, wind=wind,
        channel_set=args.channel_set, aux_set=args.aux_set)
    _, _, shape_test_loader, _, _, _, _ = _prepare_dataloaders(
        density, alpha_1, alpha_2, r_1, shape_params['seq_len'], lead_time_steps,
        shape_params['batch_size'], 'shape', shuffle_seed=args.seed, wind=wind,
        channel_set=args.channel_set, aux_set=args.aux_set)
    ...
    if n_valid <= 0:
        raise ValueError("No overlapping forecast-start range between the hs and shape test splits.")

    rel_indices = select_indices(n_valid, args.n_samples, args.index, args.random, args.seed)
    t0_values = [t0_min + r for r in rel_indices]
    print(f"Inspecting {len(t0_values)} combined sample(s) out of {n_valid} valid: {t0_values}")

    if args.save_metrics:
        print(f"Running full test-set evaluation ({n_valid} valid forecast starts)...")
        pred_np, true_np, pers_np, _, _ = eval_combined(
            project_root, args.experiment, 1, args.lead, args.seed,
            density, alpha_1, alpha_2, r_1, wind, freqs, device,
            args.channel_set, args.aux_set)
        full_metrics = compute_density_metrics(pred_np, true_np, pers_np, freqs_np)
        save_metrics_json(project_root, args.experiment, 'combined',
                          args.lead, args.seed, args.channel_set, args.aux_set, full_metrics)
    ...
    with torch.no_grad():
        for t0 in t0_values:
            ...
            X_shape, aux_shape = X_shape.unsqueeze(0).to(device), aux_shape.unsqueeze(0).to(device)

            hs_pred = hs_model.infer(X_hs, freqs, lead_time_steps,
                                     freq_means=hs_freq_means, aux=aux_hs)          # (1, L, 1) metres
            shape_pred = shape_model.infer(X_shape, freqs, lead_time_steps,
                                           freq_means=shape_freq_means, aux=aux_shape)  # (1, L, F) unit-area
            ...
            hs_pred_combined, tm02_pred = compute_bulk_params(pred_phys[np.newaxis], freqs_np)
            hs_true, tm02_true = compute_bulk_params(true_phys[np.newaxis], freqs_np)
            hs_pers, tm02_pers = compute_bulk_params(pers_phys[np.newaxis], freqs_np)
            hs_pred_combined, hs_true, hs_pers = hs_pred_combined[0], hs_true[0], hs_pers[0]
            tm02_pred, tm02_true = tm02_pred[0], tm02_true[0]
            ...
            print(f"  shape ∫ per step (should be ~1): "
                  + ", ".join(f"{v:.3f}" for v in shape_m0))
            print(f"  Hs from hs-model direct: " + ", ".join(f"{v:.3f}" for v in hs_pred_np))
            print_bulk_table(t0, lead_time_steps, hs_true, hs_pred_combined, hs_pers, tm02_true, tm02_pred)

            n_snap = min(args.n_steps, lead_time_steps)
            steps = sorted(set(np.linspace(0, lead_time_steps - 1, n_snap, dtype=int).tolist()))
            fig = plot_density_sample(t0, freqs_np, steps, pred_phys, true_phys, pers_phys,
                                      hs_pred_combined, hs_true, hs_pers, title_prefix='t0')
...
def main():
    ...
    if args.target == 'combined':
        run_combined(args, project_root, density, alpha_1, alpha_2, r_1, wind, freqs, freqs_np, device)
    else:
        run_single(args, project_root, density, alpha_1, alpha_2, r_1, wind, freqs, freqs_np, device)
