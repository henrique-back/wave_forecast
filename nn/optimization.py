...
def objective(trial, *, density, alpha_1, alpha_2, r_1, freqs, lead_time, target,
    ...
    # DataLoaders — generator seeded per trial so shuffle order differs between
    # trials (using trial.number) while remaining reproducible within each trial.
    g = torch.Generator()
    g.manual_seed(trial.number)
    ...
    # stopping at patience=10, a trial running to epoch ~20 will have tf_ratio
    ...
    for epoch in range(num_epochs):
        ...
        if val_score > best_val_score:
            ...
            # Snapshot the weights at this epoch so test evaluation uses the
            # best checkpoint, not whatever the last epoch produced.
    ...
    if best_val_metrics is not None:
        scalar_keys = ['RMSE', 'MAPE', 'CC', 'Bias', 'R2', 'overall_SS']
